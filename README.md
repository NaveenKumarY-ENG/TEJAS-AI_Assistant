...
![TEJAS demo — asking about the weather in chat, showing the hologram react through thinking/tool-call/responding states, the live tool-call pill, and the streamed reply](screenshots/demo.gif)
...
Knowledge base search runs automatically on every turn (`agent/loop.py`), the same way conversation memory recall does — never dependent on the model deciding to call `search_knowledge` itself, which repeated live testing showed a small local model would unreliably skip for exactly the questions that matter most (a bare filename reference). A document's exact extracted field table is appended to the reply verbatim by the code, not retyped by the model — also confirmed live that a 7B model would "helpfully" alter an illegible OCR value into a cleaner-looking guess even when explicitly told to present it as-is, which is worse than showing the honest, imperfect original. "What's in my knowledge base" is answered directly too, from a lightweight ambient document listing, without needing a tool call. And a knowledge-grounded answer is never written to long-term memory (same principle as the point-in-time-fact exclusion above) — the documents it's grounded in can change (re-extraction, a retag, a deletion), and a memorized old answer has no way to know that happened. This covers "what's in my knowledge base" too, not just content questions — confirmed live as a real, separate gap: that question is answered from the ambient document listing rather than a `search_knowledge` match, so it fell through the content-only version of this guard, and a stale listing answer (naming documents since deleted) kept resurfacing until the guard was widened to catch it (`agent/loop.py`'s `_is_knowledge_listing_query`).
  ...
  The Knowledge Base page (its own tab in the sidebar, not a chat overlay) also does a fair amount beyond chunk-and-search: chunking respects real sentence/word boundaries instead of a raw fixed-size character split (a real bug found live — the old approach could start or end a chunk mid-word, e.g. "nefits are shared" instead of "benefits are shared," corrupting both search results and whatever a chat answer quoted from them). Search itself is hybrid, not purely semantic — an exact-term match (an invoice number, an error code, a proper noun) that embeddings alone tend to blur is caught by a keyword-overlap signal running alongside the usual distance threshold. A PDF chunk carries its real page number end to end, so both the in-app search results and a chat citation can say "(page 4)" instead of just naming the file. The original uploaded file's bytes are kept (not just its extracted text), so a document can be **downloaded again** or **previewed in full** (a dedicated panel shows the complete extracted text plus an on-demand, one-pass local-LLM **summary**) at any time after upload, and structured-field extraction can be **re-run** on demand without re-uploading. A **note can be edited in place** — new title/text re-chunked and re-embedded under the same document id, rather than delete-and-recreate losing its position and tags. **Bulk select** across the document list drives bulk tagging/deletion, with a native tag-autocomplete (a `<datalist>` sourced from tags that already exist) everywhere a tag gets typed, so "AI"/"ai"/"A.I." don't silently fragment into separate filter pills from typos alone. Uploading byte-identical content twice is flagged with a dismissible, non-blocking warning (never a rejection — a deliberate re-upload to pick up a pipeline improvement, like the chunking fix above, must keep working) rather than silently doubling storage and search noise. A watched folder can be scoped with **include/exclude glob patterns** (`*.pdf,*.docx` / `archive/*`), so pointing it at a broad real folder doesn't recursively ingest everything under it. A 25MB upload cap and a 22-case retrieval eval suite (`tests/test_knowledge_eval.py`, run as real pytest — a fixed corpus plus real query→answer pairs) guard against, respectively, an unbounded processing cost and the next version of the chunking-boundary bug shipping unnoticed.
...
- **Amazon shopping** — defaults to amazon.in (this app's actual target user; the underlying tools worked against amazon.com until this was corrected — the storefront a real user needs is the one that ships to and takes payment from where they actually are). Tier 1: "show me some phones on Amazon" opens a real, visible browser window (a dedicated Playwright-driven Chromium profile — never your everyday Chrome, no login needed for search) navigated to a live Amazon search, and returns up to 10 real results (title/price/rating/link) in chat (`tools/shopping_tool.py`). "Open Amazon"/"open amazon.in" with nothing specific in mind yet opens the plain homepage instead of searching for an empty term — a real gap confirmed live before this: an empty search used to come back with a misleading "couldn't read any listings ... Amazon may have changed its layout" as if something were broken, when the user just wanted to browse. A price constraint ("headphones under 1000," "between 500 and 1500") is applied as Amazon's own real price-range filter (the exact `rh=p_36:...` URL parameter, confirmed live directly from Amazon.in's own filter-sidebar links, not guessed) and then double-checked in code against every returned result — confirmed live as a real, worthwhile check: a free-text "under 1000" query alone (no real filter, just hoping Amazon's search box parses the phrase) returned real results up to ₹1,490, mostly sponsored listings ranked outside the stated budget. Title extraction survives a real Amazon layout quirk found live: search cards render TWO `<h2>` elements (a brand-only mini heading first, the actual full title second, marked with `aria-label`) — a plain first-match selector silently returned just the brand name ("OnePlus") for every result until this was caught and fixed.
  ...
  Tier 2: "order the [item]" (`tools/order_tool.py`) adds a specific item to your cart and walks to Amazon's real checkout summary page, then stops — it never clicks "Place your order" or selects a payment method, because neither capability exists in the code, not just something the model is told not to do. You can order three ways: from a prior `shop_amazon` result already in the conversation; by giving the product's full name/title with nothing else (e.g. reading it straight off a listing) — resolved via a real live search and matched against the closest title, not blindly the first result; or by sharing a real Amazon.in/amzn.in product link directly, even one that never came from a `shop_amazon` call in this conversation. A link that isn't genuinely Amazon is still refused outright, and even a link that passes that check is confirmed live — a real `#productTitle` must actually load — before anything else happens, so a hallucinated-but-plausible-looking link fails safely instead of silently proceeding. Every successful add states the product's real name, price, and link alongside the added-to-cart confirmation, not just a bare checkout summary. Adding to cart is confirmed by actually watching Amazon's own cart-count change, not just that the click didn't raise an error — a blocked click (a size/color prompt, a promo interstitial) used to be silently reported as success; a further live-found quirk (some listings render two elements sharing `id="add-to-cart-button"`, one permanently hidden) is also handled, clicking whichever one is actually visible. The final confirmation also flags it if the product's price has moved since the earlier search (Amazon prices shift constantly — flash deals, stock-based pricing) instead of letting a stale price go unmentioned right before a real cart decision. Amazon's real checkout funnel was confirmed live to be multi-step — a "Need anything else?" upsell page, then a "Secure checkout" summary with delivery address, order total, and payment method selection — and the tool stops at that summary rather than pushing further, since choosing a payment method is itself something this app must never do on your behalf. "What's in my cart" (`tools/cart_tool.py`, the `view_amazon_cart` tool) reads your real, live Amazon cart and lists every item's title/price/quantity/link — scoped to the actual active cart, not the separate "Saved for Later" list, which shares the same underlying markup but isn't really in your cart. "Remove the [item] from my cart" (`remove_from_amazon_cart`, same file) does the reverse — matched by name (order-independent word matching, so "Nataraj pen jar" still finds a real title like "Nataraj GCM Ball Pen Jar") or by the exact link from a prior `view_amazon_cart` result, refusing outright rather than guessing if zero or more than one item matches. A removal is confirmed by re-loading the real cart page fresh and checking the item is actually gone (with a short retry, since Amazon's own delete is an async server-side operation and an immediate re-check can occasionally still show the pre-delete state) — never just that the click didn't raise an error. Requires being logged into Amazon in that dedicated browser window once (session persists after that); if you're not, every one of these tools says so plainly rather than trying to sign in on your behalf. See [Security notes](#security-notes) for why full autonomous purchasing is a deliberate non-goal.
- **Live.AI** (sidebar → Live.AI) — the real-time web/shopping view, added after a reported bug turned out to be a genuine gap: nothing in the codebase ever navigated a browser anywhere but amazon.in, so "open flipkart.com" (or any other site) silently had no tool that could act on it. `open_website` (`tools/browse_tool.py`) fixes that generically — a name ("flipkart", "youtube") or a bare/full URL opens in the same shared Playwright browser `shop_amazon` uses, with a small alias list for common sites and a best-effort `.com` fallback for anything else; it rejects non-http(s) schemes outright (`javascript:`/`data:`/`file:` don't resolve to a real page navigation). `read_webpage` (`tools/read_webpage_tool.py`) fetches a URL server-side and returns its readable text (reusing the same BeautifulSoup extraction Knowledge Base URL ingestion already used, now shared via `integrations/html_extract.py`) for research/summarizing — guarded against SSRF by resolving the hostname and refusing to fetch a private/loopback/link-local address before making the request, since this one *is* a server-side request unlike `open_website`'s local browser window. `shop_flipkart` (`tools/flipkart_tool.py`) searches Flipkart the same way `shop_amazon` searches Amazon — but is **research-only by design**: unlike Amazon, there's no existing anti-bot tuning or cart integration for Flipkart in this codebase, so rather than fake an add-to-cart/checkout step, it says so plainly and offers to open the product page instead (same "never claim an action that didn't happen" rule `order_amazon` already follows). The Live.AI page itself reuses the exact same chat/session/WebSocket Home uses (there's genuinely only one conversation in this app) with different chrome: a live status strip reflecting real backend checks (web search configured? browser tools installed?), suggested research/shopping prompts, and a sidebar card naming what's actually supported. The whole feature is a real, working kill switch — `LIVE_AI_ENABLED=false` disables all three tools (they self-report disabled rather than erroring) and hides the sidebar entry entirely, with zero effect on anything else in the app.
- **Settings** (sidebar → Settings) — a real, mostly read-only control center, not a mock settings screen: General, AI & Models, Voice & Speech, Chat & Memory, Knowledge, System, Privacy & Data, and Developer. The critical property is that **AI & Models and Voice & Speech read/write the exact same Zustand store fields and `GET`/`POST /api/models`/`/api/tts/voices` endpoints the top bar's own model/voice selectors already use** — switching a model or voice here calls the identical endpoint and store setter those components call, so the top bar and Settings can never disagree (verified live with a real round-trip: switch in Settings → top bar updates; switch back in the top bar → Settings updates). Everything else is either a real fact traced to something that already exists (SQLite/ChromaDB health from a new lightweight `GET /api/system/health` check, document/folder counts from the existing Knowledge Base endpoints, cloud-provider configuration from `/api/meta`) or explicitly **omitted** rather than faked — "TEJAS Version" doesn't show `package.json`'s meaningless `"0.0.0"`. No dangerous actions (no restart/delete/clear-database controls) — this page is read-mostly, with model/voice switching and a session-only voice-output mute toggle as the only real actions.
...
flowchart LR
    ...
    Loop --> Tools["Tool system<br/>tools/ (16 tools)"]
    Tools --> Shopping["Amazon shopping<br/>shop / order / cart"]
    Tools --> LiveAI["Live.AI<br/>open_website / read_webpage / shop_flipkart"]
...
> **Known limitation**: the sidebar doesn't collapse or hide at real mobile widths (~390px) — it renders at full size and pushes the actual content into an unusably narrow strip. This app is built and tested for desktop use; a proper phone layout (auto-collapse, a hamburger drawer, or a bottom tab bar) is a real gap, not yet decided between the options, rather than something silently broken.
...
│   ├── cart_tool.py          # Reads/removes items from the real, live Amazon cart
│   ├── browse_tool.py        # Live.AI: open any website by name/URL in the shared browser
│   ├── read_webpage_tool.py  # Live.AI: fetch + extract readable text from a URL (SSRF-guarded)
│   └── flipkart_tool.py      # Live.AI: Flipkart product search via a real browser (research only)
├── memory/
...
│   ├── browser.py            # Persistent Playwright/Chromium context shared by Amazon/Flipkart/open_website
│   └── html_extract.py       # Shared readable-text extraction — used by Knowledge Base URL ingest + read_webpage
├── frontend/                 # React/Three.js dashboard (Vite)
│   └── src/
│       ├── components/       # core/ (hologram), ui/ (chat, widgets), layout/, voice/, knowledge/, liveai/, settings/
...
| `GEMINI_MODEL` | `gemini-3.6-flash` | Gemini model name. `gemini-2.5-flash` has been retired for new API keys (Google's own 404 points to `gemini-3.6-flash` as the replacement). **Update:** this README previously said `gemini-3.6-flash` was chosen specifically to avoid `3.7-flash`'s tight 20-requests/day free-tier cap — re-confirmed live (2026-09-06) that `gemini-3.6-flash` now hits the exact same `RESOURCE_EXHAUSTED` / 20-requests/day limit. Google's free-tier quotas appear to apply uniformly across these models now, not just the newest one — treat any Gemini free-tier use as tight, and expect to hit the cap during a normal test/dev session, not just under heavy load |
...
| `LIVE_AI_ENABLED` | `1` | Set to `0`/`false` to disable Live.AI entirely — `open_website`/`read_webpage`/`shop_flipkart` all self-report disabled, and the sidebar's Live.AI entry disappears. Nothing else in the app is affected either way |
...
The model decides when to call these — it doesn't guess at things it can look up. Kept deliberately low (16, not more): on CPU-only local inference, every tool in the schema adds real, measured latency to *every* request (see [Performance notes](#performance-notes) below), so related actions are grouped into one tool with an operation/action parameter rather than split into many single-purpose ones. `search_knowledge`, `shop_amazon`, `order_amazon`, `view_amazon_cart`, `remove_from_amazon_cart`, `open_website`, `read_webpage`, and `shop_flipkart` are exceptions worth their slot — none of those capabilities exist at all without their own tool.
...
| `remove_from_amazon_cart` | Remove one specific item from your real cart, by name or exact link. Refuses (and shows what's actually there) rather than guessing if the name matches zero or more than one item — the same "never guess" principle as `order_amazon`'s variant matching |
| `open_website` | Open any website by name or URL (e.g. "flipkart", "youtube", a bare domain, or a full URL) in the same shared browser window `shop_amazon` uses. For just opening/browsing a site — not for searching/shopping on Amazon or Flipkart, which have their own dedicated tools |
| `read_webpage` | Fetch a URL server-side and return its readable text (boilerplate stripped) for research/summarizing. Guarded against SSRF — refuses to fetch a private/loopback/link-local address |
| `shop_flipkart` | Search Flipkart.com for a product and return up to 10 real results, same shape as `shop_amazon`. **Research only** — cannot add to cart or purchase anything; says so plainly if asked to |
...
| `GET /api/meta` | Assistant name, active model, full tool list (name + description), whether neural TTS is available (`tts_available`), whether OCR is available (`ocr_available`), Live.AI's real capability checks (`live_ai_enabled`, `browser_available`, `search_configured`), and whether Anthropic/Gemini look configured (`anthropic_configured`, `gemini_configured` — booleans only, the key itself is never returned) — what the UI header/status panel, the Live.AI page, and Settings' Privacy & Data section all read on load. |
| `GET /api/system/health` | Read-only diagnostics for Settings' System section: `{"status", "database", "vector_store", "python_version"}`. Checks only SQLite (a trivial query) and ChromaDB (`heartbeat()`, a free liveness check) — no embeddings, no LLM calls, no writes. A real failure reports `"unavailable"` rather than crashing the endpoint. |
...
| `GET /api/knowledge` | List uploaded knowledge-base documents (id, filename, chunk count, tags, source type, upload date, `has_file`). A URL-ingested entry's "filename" is the URL itself; a note's is its title. Deliberately excludes each document's full extracted text (see the next row) — sending that on every page load/poll would be pure waste. |
| `GET /api/knowledge/{id}` | Single-document detail, including the full extracted text (`raw_text`) — what the Knowledge Base page's preview panel reads. 404 if it doesn't exist. |
| `GET /api/knowledge/{id}/file` | Download the original uploaded file's bytes. 404 if this document has none stored (notes/URLs never do — there's no "original file" separate from their own text — and anything uploaded before this existed also won't have one). |
| `POST /api/knowledge` | Multipart file upload (`file` field, `.txt`/`.md`/`.pdf`/`.docx`/`.png`/`.jpg`/`.jpeg`, max 25MB, optional `tags` field — comma-separated) → chunked, embedded, and indexed; the original bytes are also kept (enables the two rows above). Images and scanned/image-only PDF pages are OCR'd (`memory/ocr.py`) when OCR is available. Returns the new document's metadata plus `duplicate_of` (`{id, filename}` of an existing document with byte-identical content, or `null`) — informational only, never blocks the upload. 400 for an unsupported type, a file over the size cap, no extractable text, or OCR being unavailable for an image. |
...
| `PATCH /api/knowledge/{id}/note` | Body `{"title": "...", "text": "...", "tags": [...] \| null}` → replaces a note's title/text in place, re-chunked and re-embedded under the same document id (tags left alone if omitted). 404 if it doesn't exist, 400 for an empty title/text. |
| `POST /api/knowledge/{id}/reprocess` | Re-run structured field extraction against a document's stored text, without re-uploading it. 404 if it doesn't exist, 400 if it predates `raw_text` being stored (nothing to reprocess from — re-upload it instead). |
| `POST /api/knowledge/{id}/summarize` | One-pass local-LLM summary of a document's stored text (bounded input size — a genuinely book-length document gets a summary of its first section, not the whole thing). 404 if it doesn't exist, 400 if it has no stored text or the local model call fails. |
| `PATCH /api/knowledge/{id}/tags` | Body `{"tags": [...]}` → replaces a document's tags in place. 404 if it doesn't exist. |
| `GET /api/knowledge/search?q=...` | Hybrid semantic + keyword search — the same one `search_knowledge` (the tool) uses, exposed directly to power the Knowledge Base page's search box. Returns `{"results": [{filename, text, page}, ...]}` — `page` is the PDF page number a chunk came from, or `null` for anything without a real page concept. |
| `DELETE /api/knowledge/{id}` | Delete a document and all its chunks (and its stored original file, if any). 404 if it doesn't exist. 400 if it's managed by a watched folder — unwatch the folder or remove the file instead. |
| `GET /api/knowledge/folders` | List watched folders (id, path, live file count, include/exclude patterns). |
| `POST /api/knowledge/folders` | Body `{"path": "...", "include_pattern": "", "exclude_pattern": ""}` → start watching a folder on this machine (`memory/folder_watch.py`); an initial scan ingests what's already there, then new/changed/deleted files stay in sync live. The optional patterns are comma-separated globs (e.g. `*.pdf,*.docx` / `archive/*`) matched against each file's path relative to the folder, so a broad folder doesn't have to mean "ingest everything under it." 400 if the path isn't a real folder or is already watched. |
...
433 tests as of the last full audit, covering every tool (including sandbox-escape/path-traversal attempts against `file_ops`, and a live-reproduced regression guard for a bug where the code-execution tool's temp script briefly broke the dev server's autoreload — see `tests/test_code_exec.py`), the knowledge base's ingestion/search/reprocessing/note-editing paths (including the single-word/filename-stem search fixes above), reminders' full CRUD + Calendar sync, Amazon shopping's variant-matching/checkout-safety logic, Live.AI's site-resolution/SSRF-guard/research-only-honesty logic (`tests/test_browse_tool.py`, `tests/test_read_webpage_tool.py`, `tests/test_flipkart_tool.py`, `tests/test_html_extract.py`), the shared browser's auto-heal-on-close retry logic (`tests/test_browser.py`), Settings' new `/api/system/health` endpoint and `/api/meta` fields, and the Gemini tool-loop regression above (`tests/test_llm_client.py`). `tests/test_knowledge_eval.py` is a small retrieval quality eval harness — a fixed 8-document corpus plus 22 real `(query, expected answer)` pairs (including true negatives), run as ordinary pytest — built specifically so a future regression in chunking or relevance scoring fails a test automatically instead of needing a human to notice it live.
...
- After a tool call succeeds, the system prompt requires the reply to state the actual result — the number, fact, or value itself — not just describe that a tool was called. Confirmed live as a real, model-specific gap: asked to calculate something, `llama3.1:latest` would successfully call `execute_python` and then reply only "I called the execute_python tool to calculate the result," never actually saying what the answer was. The rule fixed this reliably for calculations; the same model can still occasionally do it for a `get_weather` result specifically — a real, live-confirmed limitation of that model rather than something prompt wording alone fully closes, and not something seen on the default `qwen2.5:7b`.
- Gemini needed its own separate fix for a related but distinct failure: asked to calculate something, `gemini-3.6-flash` got the correct answer from `execute_python` on the very first call, then called the *identical* tool with the *identical* input 7 more times — burning the entire tool-call iteration cap — without ever emitting a real reply. Root cause, confirmed against the actual SQLite-persisted conversation history: `agent/llm_client.py`'s `_gemini_contents` sends a synthetic "Please continue." message after folding a tool result into the history (required, since Gemini rejects a request ending on a "model" turn) — and that phrasing reads as "keep taking actions" to a tool-calling model, exactly the wrong instruction right after a tool already answered the question. Replaced with an explicit "give your final answer now, don't call the same tool again" directive.
...
A live side-by-side of every non-Anthropic model on the same 5-prompt battery (arithmetic, a mixed expression, a word problem, division by zero, and a weather lookup): `qwen2.5:7b` (the default) was both fastest and fully correct throughout. `qwen3:8b` was also fully correct but noticeably slower (its "thinking" mode can take 50-65s for a single reply — budget for that if you switch to it). `gemma2:9b` (no tool access, by design — see below) reasons basic arithmetic correctly from its own training, as expected, but can only acknowledge it can't fetch live data like weather, not actually get it. `gemini-3.6-flash` was correct on the two calls it got through before hitting its real free-tier cap (see [Configuration](#configuration)).
...
- `read_webpage` is the one Live.AI tool that makes the *server itself* issue an outbound request (unlike `open_website`, which just points a real, visible local browser window the user can already see) — it resolves the target hostname first and refuses to fetch anything that resolves to a private, loopback, link-local, or reserved address, a basic SSRF guard (`tools/read_webpage_tool.py`'s `assert_public_host`). `open_website` itself rejects any scheme other than `http`/`https` (`javascript:`, `data:`, `file:`, ...), so it can only ever navigate to a real web page, never a local/script URL, regardless of what a user or an untrusted page's content asks for.
- `shop_flipkart` is deliberately research-only — no add-to-cart or checkout code path exists for it at all (same "the capability doesn't exist to be misused" principle as `order_amazon`'s missing "place order" step above), and its system prompt rule (`config.py`) tells the model to say so plainly rather than imply otherwise if asked to buy something on Flipkart or any other non-Amazon site.
- Web content (a page `read_webpage` fetches, a snippet `web_search` returns) is always treated as data the model reasons *about*, never as instructions — the system prompt makes no exception for text that looks like an instruction embedded in a fetched page, and none of the Live.AI tools execute anything a fetched page contains (no script execution, no following an embedded "click here" instruction on their own).
- The shared Playwright browser (`integrations/browser.py`, used by Amazon shopping and every Live.AI browsing tool) heals itself if the visible window is closed manually: closing it used to leave every subsequent tool call failing until the whole server was restarted, since the browser process exits on its own when its last window closes but the app's cached reference had no way to know that. `new_page()` now detects a dead browser and relaunches automatically, retrying with a short pause between attempts (a still-closing browser process can hold its own profile lock for a moment) — confirmed live, reusing the same on-disk profile so Amazon login state survives the relaunch.
- `data/logs/tejas.log` is a rotating log file (capped size, a few backups) capturing the same detail every tool's own error handling already produces server-side — never anything from a request/response body, so no secret or personal data ends up there beyond what was already being logged to the console.
