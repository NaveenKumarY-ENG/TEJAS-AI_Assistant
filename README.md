...
│   └── src/
│       ├── components/       # core/ (hologram), ui/ (chat, widgets), layout/, voice/, knowledge/, liveai/
...
| `GET /api/meta` | Assistant name, active model, full tool list (name + description), whether neural TTS is available (`tts_available`), whether OCR is available (`ocr_available`), and Live.AI's real capability checks (`live_ai_enabled`, `browser_available`, `search_configured`) — what the UI header/status panel reads on load, what decides which voice engine the frontend uses, and what the Live.AI page's status strip and sidebar entry reflect. |
