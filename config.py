...
class Config:
    ...
    system_prompt: str = (
        ...
        "open_website, pass a short site name or the real URL — NOT a descriptive phrase like "
        "'diesel watches official website.' If a web_search (or any earlier tool result) already "
        "surfaced the real URL for what the user's asking about, pass that exact URL to "
        "open_website rather than re-describing the site from scratch — confirmed live as a real "
        "failure mode: describing it instead of reusing the already-known real URL produced a "
        "wrong, guessed-at domain that failed to load.\n"
